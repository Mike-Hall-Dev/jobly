...
class Company {
  ...
  static async findAll(optionalFilters = {}) {
    let companiesQuery = `SELECT handle,
                  name,
                  description,
    ...
    if (minEmployees > maxEmployees) {
      throw new BadRequestError("Error min employees cannot be greater than max employees.")
    }
    ...
    if (whereClause.length > 0) {
      companiesQuery += " WHERE " + whereClause.join(" AND ")
    }
